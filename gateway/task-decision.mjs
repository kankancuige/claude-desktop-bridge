const WRITE_SIGNALS = /(?:修改|修复|实现|新增|添加|增加|加上|加个|加一个|补上|补个|补一个|加入|移除|删除|重构|改代码|写代码|补代码|优化|完善|处理(?:一下)?|解决(?:一下|掉)?|完成(?:这个)?(?:需求|功能)?|做出来|落地|应用补丁|执行命令|运行测试|构建项目|编译项目|提交代码|commit|push|部署|安装依赖)/i
...
function workflowFor(text, action) {
    ...
    if (BUG_SIGNALS.test(text) && action !== 'implement') return 'bug-hunter'
    if (PLAN_SIGNALS.test(text)) return 'judge-panel'
    if (RESEARCH_SIGNALS.test(text)) return 'deep-research'
    if (action === 'implement' && BUG_SIGNALS.test(text)) return 'generate-critic-fix'
    if (action === 'refactor') return 'default'
    return 'none'
}
...
export function decideTask(input = {}) {
    ...
    else if (action === 'implement' || action === 'operate' || action === 'refactor' || action === 'review') finalReview = 'balanced'
