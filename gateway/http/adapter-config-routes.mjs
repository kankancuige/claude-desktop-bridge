...
export function createAdapterConfigRoutes(deps = {}) {
    ...
    const getNotificationRepository = typeof deps.getNotificationRepository === 'function'
        ...
        : () => null
    const getPersistedPairedUserCount = typeof deps.getPersistedPairedUserCount === 'function'
        ? deps.getPersistedPairedUserCount
        : () => 0
    ...
    if (req.method === 'GET' && url.pathname === '/api/config/scheduled-tasks') {
        const list = Object.entries(scheduledTaskStore?.list?.() || {}).map(([id, t]) => ({
            ...
            permissionMode: t.permissionMode || 'bypassPermissions', maxTurns: t.maxTurns || 20,
    ...
    if (req.method === 'POST' && url.pathname === '/api/config/scheduled-tasks') {
        ...
        const permissionMode = b.permissionMode || 'bypassPermissions'
    ...
    if (req.method === 'POST' && notificationRetryMatch) {
        ...
        const result = await hook.retryNotifications()
        res.writeHead(202)
    ...
    if (req.method === 'GET' && url.pathname === '/api/config/adapters') {
        ...
        const bindingStatus = p => {
            ...
            const livePaired = getAdapterHook(p)?.pairedUserCount?.()
            const paired = Number(livePaired ?? getPersistedPairedUserCount(p))
            return {
                total: values.length,
                ...
                paired: Number.isInteger(paired) && paired >= 0 ? paired : 0,
            }
        }
        res.writeHead(200);
        res.end(JSON.stringify({
            ...
            platforms: [{
                ...
                pairedUserCount: bindingStatus('wechat').paired,
                ...
                pairedUserCount: bindingStatus('feishu').paired,
                ...
                pairedUserCount: bindingStatus('dingtalk').paired,
    ...
    if (req.method === 'POST' && url.pathname === '/api/confirm') {
        const b = await readBody(req);
        const {sessionId: sid, requestId, decision, optionIndex, questionIndex, customText, answers} = b
        ...
        const result = entry.type === 'choice'
            ? decisionToResult(entry, null, optionIndex, questionIndex, customText, answers)
        ...
        if (result?.incomplete) {
            ...
            res.writeHead(409)
            res.end(JSON.stringify({ok: false, reason: 'confirmation_incomplete', message: result.message || '请完成所有问题后再提交', answers: result.answers || entry.input?.answers || {}}))
            return
        }
    ...
    if (req.method === 'POST' && url.pathname === '/api/sessions-by-label') {
        ...
        const identity = getAdapterIdentity(req)
        if (!identity) {
            res.writeHead(403); res.end(JSON.stringify({error: 'adapter identity required'})); return
        }
        ...
        res.writeHead(200); res.end(JSON.stringify({
            ok: true,
            label: b.label,
            sessions: projectSessions.map(s => ({id: s.id, title: s.title})),
        })); return
    }
