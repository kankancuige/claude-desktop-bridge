 ...
 *          → TaskCommand 进程内提交 → HTTP API 回复。
 * 关键数据流: 钉钉 Stream WS 推送 → DWClient → registerCallbackListener(TOPIC_ROBOT)
 *          → handleMessage() → resolve session → runImTask() → sendMsg() HTTP 回复
 *
...
export function startDingTalkAdapter(token, {taskCommands, repository, onNotificationStateChange = null} = {}) {
    ...
    async function injectAndWait(sessionId, userId, text, messageId = '') {
        ...
        return runImTask({
            ...
            onError: (error, context) => {
                log.warn({err: error, sessionId: sessionId?.slice(0, 8), ...context}, '钉钉 IM 任务处理失败')
            },
        })
    }
    ...
    function findUserForSession(sid) {
        ...
        return findLatestAdapterUserForSession(ad, 'dingtalk', sid, pairedUsers)
    }
    ...
    function onConfirmResolved(sessionId, requestId, resolution = {}) {
        ...
        if (!removed || resolution.wonBy === 'dingtalk') return
    ...
    dwClient.registerCallbackListener(TOPIC_ROBOT, async (message) => {
        ...
        try {
            ...
            const eventId = normalizeImMessageId(message.headers?.messageId || body.msgId || body.messageId)
