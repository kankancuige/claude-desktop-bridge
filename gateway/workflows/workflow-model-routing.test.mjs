...
test('普通实施和普通异常问句不启动 Workflow，明确审查与调研才启动', () => {
    ...
    assert.equal(shouldAutoTriggerWorkflow({action: 'query', contextProfile: 'full', workflow: 'bug-hunter'}), false)
    assert.equal(shouldAutoTriggerWorkflow({action: 'query', contextProfile: 'full', workflow: 'deep-research'}), true)
