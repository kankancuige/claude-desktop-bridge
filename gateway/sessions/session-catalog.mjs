...
/**
 * 增量协调一个项目的 transcript 元数据到 SQLite 派生目录。
...
export function reconcileSessionCatalog({
    ...
    const previousById = stateStore?.available
        ? stateStore.getSessionCatalogs(projectKey, limitedCandidates.map(item => sessionIdFromFile(item.filename)))
        : new Map()
    for (const {filename, transcriptPath, stat} of limitedCandidates) {
        ...
        const visibleFromSqlite = previous?.visibility === 'visible'
        ...
        if (!sessionId || (!shouldShowSession(visibility, sessionId) && !visibleFromSqlite && !repairedLegacyMain)) continue
    ...
    if (stateStore?.available) {
        stateStore.upsertSessionCatalogBatch(rows)
        for (const indexed of stateStore.listSessionIndex(projectKey, {limit: 500})) {
            if (!seenPaths.has(indexed.transcriptPath)) stateStore.removeSessionIndex(indexed.transcriptPath)
        }
    }
