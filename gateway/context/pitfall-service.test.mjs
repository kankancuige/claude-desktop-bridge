...
test('首次观察、重复指纹、项目隔离和冷却期', t => {
    let timestamp = 1000
    const store = new BridgeStateDb({bridgeHome: mkdtempSync(join(tmpdir(), 'pitfall-service-')), now: () => timestamp})
    t.after(() => store.close())
    const service = createPitfallService({stateStore: store, now: () => timestamp, cooldownMs: 100})
...
test('确认、缓解、过期和相关性过滤', t => {
    let timestamp = 1000
    const store = new BridgeStateDb({bridgeHome: mkdtempSync(join(tmpdir(), 'pitfall-lifecycle-')), now: () => timestamp})
    t.after(() => store.close())
    const service = createPitfallService({stateStore: store, now: () => timestamp})
