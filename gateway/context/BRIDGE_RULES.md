...
- 当任务明确涉及数字孪生、工业孪生、机器人孪生、设备三维资产、CAD/GLB 节点绑定、遥测驱动模型状态或 twin manifest 时，按需加载 `digital-twin-cad` Skill；普通 CAD、普通前端和仅 Viewer 预览不加载。
- 数字孪生项目的单位、坐标系、资产注册、状态传输和设备映射优先读取项目根目录的 `twin.config.yaml` 或项目真实约定；没有项目配置时使用 Skill 默认值，不复制或读取外部全局规则。
