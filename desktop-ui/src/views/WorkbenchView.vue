      ...
      <article class="detail-section task-detail">
        ...
        <template v-else>
          ...
          <div class="detail-grid">
            ...
            <div class="detail-block session-link-block"><h3>关联会话</h3><div v-if="taskDetail?.sessionLink?.available"><div class="evidence-line"><span>Session</span><b>{{ taskDetail.sessionLink.sessionId }}</b></div><div class="evidence-line"><span>回合</span><b>{{ taskDetail.sessionLink.turnId || detailTask.turnId || '未记录' }}</b></div><button class="text-button" type="button" @click="openSession({...detailTask, sessionId: taskDetail.sessionLink.sessionId || undefined, projectKey: taskDetail.sessionLink.encodedDir || detailTask.projectKey || undefined, turnId: taskDetail.sessionLink.turnId || detailTask.turnId || undefined})">打开对应会话</button></div><div v-else class="muted">对应会话暂不可用，任务数据仍可查看。</div></div>
          </div>
        </template>
      </article>
