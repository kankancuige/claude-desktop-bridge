...
|---|---|---|---|---|---|---|
| 方案确认与 Provider 继续 | AskUserQuestion 或普通权限请求已由桌面/IM 确认 | `confirmation_response=confirmed` 只表示 Gateway 已结算确认；随后必须观察新的 SDK/Provider 事件，若无事件则由确认超时或 stream watchdog 收口为明确失败/可继续状态 | L3 | 脱敏确认回执、后续 SDK 事件时间线、watchdog/终态事件 | 仅凭“确认已提交”显示正在执行，或活动工具残留导致无限等待 | partially verified：2026-08-28 已通过确认 Promise、工具活动结算、动态 watchdog 和前端事实文案单测；真实 Electron/Provider 首个后续事件仍待验收 |
| Claude CLI 版本配套 | 未设置 `CLAUDE_EXE`、系统 `claudeExe` 或请求级覆盖 | Gateway 使用 Agent SDK 同版本 native binary；用户显式覆盖时使用配置值并保留失败诊断 | L2 | CLI resolver 单测、SDK/CLI 版本输出 | 自动扫描并启动用户目录旧版 CLI，造成确认后 Provider 不继续 | passed（代码门禁）：SDK `0.3.243` 配套 Claude Code `2.1.243` 优先于用户目录 `2.1.181/2.1.187` |
...
| 输入框暂停与继续 | 任务运行时保持输入为空点击暂停；终态或 `max_turns` 后再次点击输入框主按钮 | 当前生成被取消或落入 `incomplete + resumable`；输入框保持为空；按钮切换为播放图标；点击后复用同一 transcript 续跑 | L3 | 桌面录屏、`generation_stopped`/`task/continuation-paused`、后续 `task/created` 和非空回复 | 自动回填旧任务、暂停或 `max_turns` 后自动续跑、按钮仍显示停止或发送 | partially verified：2026-08-28 纯状态机、草稿投影、Vue 接线和 `max_turns` 禁止自动重建 Query 测试通过；真实 Electron/Provider 交互待验收 |
| 暂停后开始新任务 | 任务暂停后直接输入不同内容并发送 | 输入操作优先显示发送；Gateway 创建新 taskId，不恢复旧 Coordinator，不自动拼接旧任务 | L3 | 两次 taskId、Session Journal、桌面消息序列 | 新内容被当作旧任务补充、旧任务自动恢复或新内容丢失 | partially verified：2026-08-28 Gateway 既有终态后普通输入路径静态确认会走 `task/created`；真实运行序列待验收 |
| 强制关闭后恢复暂停 | 任务运行时强制结束程序，重启并打开同一历史会话 | 运行态恢复为 interrupted/stopped + resumable；即使 SDK 尚未产生 history ID 也保留任务请求、已有修改和继续入口；旧任务文本不回填；必须点击输入框播放图标才继续，直接输入新内容则开始新任务 | L3 | 重启前后 task-state、桌面录屏、同一 SDK Session ID（如存在） | 伪装为 running、自动发送旧任务、创建替代历史会话、无 history ID 时丢失继续入口 | partially verified：2026-08-28 恢复投影、自动续跑隔离和 interrupted draft 单测通过；真实强制关闭验收待执行 |
