...
| 长期 Memory | 跨任务复用稳定约定和已验证事实 | PostgreSQL `content_documents`；`memory/*.md` 为兼容副本 | 动作任务关键词匹配或明确 Memory 操作 |
...
| 成功会话自动沉淀 | 成功任务只解析用户原始请求中的明确长期约定，生成 PostgreSQL candidate；跨会话按项目+内容稳定去重，审批后才 active | 已实现 |
...
当前不强制启用 OpenViking 式分层召回，但已为单项目 Memory 增长预留扩展：正文保持 L2，`metadata` 可保存 L0/L1、Memory 类型和父节点；Repository 支持按父节点和层级读取；离线回填支持断点、取消和幂等；规模策略按数量、召回质量和注入预算给出升级建议。默认仍是扁平关键词召回，避免在小规模项目上增加 embedding 或分层运行成本。

## 借鉴与不照搬

优先借鉴：显式作用域、路径所有权校验、Agent 级隔离选项、Memory 与压缩分离、禁用 `none`、可见来源和用户撤销。

暂不照搬：自动保存完整任务摘要、从普通模型回复猜测长期事实、默认加载所有 Memory、云同步和多用户权限。这些功能会扩大敏感数据、依赖和故障面，也不符合当前单机一对一桌面端边界。
